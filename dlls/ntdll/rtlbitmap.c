 ...
 */
static int NTDLL_RunSortFn(const void * HOSTPTR lhs, const void * HOSTPTR rhs)
{
  if (((const RTL_BITMAP_RUN* HOSTPTR)lhs)->NumberOfBits > ((const RTL_BITMAP_RUN* HOSTPTR)rhs)->NumberOfBits)
    return -1;
  return 1;
}
