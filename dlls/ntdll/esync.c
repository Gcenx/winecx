...
void CDECL __wine_esync_set_queue_fd( int fd )
{
...
#endif
    int refcount;
    void * HOSTPTR shm;              /* pointer to shm section */
};
...
{
    interlocked_xchg_add( &obj->refcount, 1 );
}

static void release_object( struct esync *obj )
{
    if (interlocked_xchg_add( &obj->refcount, -1 ) == 1)
    {
        efd_close( obj );
...
static struct esync * * HOSTPTR esync_list[ESYNC_LIST_ENTRIES];
...
static struct esync *add_to_list( HANDLE handle, enum esync_type type, int fd, void * HOSTPTR shm )
{
    ...
    if (!(obj = RtlAllocateHeap( GetProcessHeap(), 0, sizeof(*obj) )))
    {
    ...
    if (!esync_list[entry])  /* do we need to allocate a new block of entries? */
    {
        ...
        {
            void * HOSTPTR ptr = wine_anon_mmap( NULL, ESYNC_LIST_BLOCK_SIZE * sizeof(struct esync),
    ...
    assert(!interlocked_xchg_ptr((void ** HOSTPTR)&esync_list[entry][idx], obj));
    return obj;
}
...
NTSTATUS esync_close( HANDLE handle )
{
    ...
    {
        if ((obj = interlocked_xchg_ptr((void ** HOSTPTR)&esync_list[entry][idx], 0)))
        {
            release_object( obj );
            return STATUS_SUCCESS;
        }
...
userapc:
    ...
    status = server_select( NULL, 0, SELECT_INTERRUPTIBLE | SELECT_ALERTABLE, &zero );
