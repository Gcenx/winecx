...
struct wined3d_settings wined3d_settings =
{
    ...
    1,              /* Try to avoid getting NaNs in < SM4 by hacking out infinites. */
...
static BOOL wined3d_dll_init(HINSTANCE hInstDLL)
{
    ...
    if (hkey || appkey)
    {
        ...
        else if (!get_config_key(hkey, appkey, "csmt", buffer, size) && !strcmp(buffer,"disabled"))
        {
...
static BOOL wined3d_dll_destroy(HINSTANCE hInstDLL)
{
    ...
    heap_free(cxgames_hacks.disabled_extensions);
