...
static struct wined3d_cs_packet *wined3d_next_cs_packet(const uint8_t * WIN32PTR data, SIZE_T *offset)
{
        ...
        struct wined3d_map_desc *map_desc, const struct wined3d_box *box, uint32_t flags)
{
    ...
    if (flags & WINED3D_MAP_NOOVERWRITE)
    {
        ...
        map_desc->data = (void *)(UINT_PTR)align((size_t)upload->sysmem, RESOURCE_ALIGNMENT);
        return true;
    }
        ...
        struct wined3d_resource *resource, unsigned int sub_resource_idx, struct wined3d_box *box, struct upload_bo *bo)
{
    ...
    if ((upload = deferred_context_get_upload(deferred, resource, sub_resource_idx)))
    {
        *box = upload->box;
        ...
        bo->addr.addr = (uint8_t * WIN32PTR)(UINT_PTR)align((size_t)upload->sysmem, RESOURCE_ALIGNMENT);
